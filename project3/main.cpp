...
int main() {
    ...
    while (true) {
        int word_size = 0;
        while (word_size <= 0 || word_size > game.get_max_word_size()) {
			word_size = get_integer("How long of a word would you like? (must fall between 0 and " + to_string(game.get_max_word_size()) + " inclusive)");
        }
        ...
        while (!game.is_won() && !game.is_lost()) {
            ...
            if (game.is_spoiled()) {
				cout << "There are " << game.get_remaining_words();
            ...
            if (good_guess) {
                ...
                game.set_remaining_guesses(game.get_guesses_remaining() - 1);
                game.add_guess(guess);
            }
